...
<body class="p-8 flex items-center justify-center min-h-screen">
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Premium Pro secret coupon code bypass
    if (tempInput.trim() === "shdmfl0504!") {
      setIsPremium(true);
      localStorage.setItem("astra_is_premium", "true");
      triggerFireworks();
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `👑 **Astra Premium Pro 등급 업그레이드 완료**\n\n비밀 코드 \`shdmfl0504!\` 인증에 성공하였습니다! 귀하의 계정이 최고 등급인 **Astra Premium Pro** (1년 무제한 이용권)로 즉시 업그레이드되었습니다.\n\n- **현재 등급**: Astra Premium Pro\n- **유효 기한**: 오늘로부터 1년 (${expiryDate.toLocaleDateString()})\n- **제공 혜택**: 이미지 팩토리, 비디오 기획 시뮬레이터, 코드 크래프트 샌드박스 등 모든 프리미엄 기능 무제한 가동 승인.\n\n지금부터 어떠한 연산 병목 없이 모든 초고성능 인공지능 도구를 무제한으로 사용하실 수 있습니다!`,
            timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
          },
        ]);
    ...
    const tempAssistantId = "ai-stream-" + Date.now();
    setMessages((prev) => [
      ...prev,
      {
        id: tempAssistantId,
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            // Keep going, line could be partial or parsed in next frame
          }
        }
      }
    } catch (error: any) {
      setMessages((prev) => {
        const targetExists = prev.some(m => m.id === tempAssistantId);
        if (targetExists) {
          return prev.map((m) =>
            m.id === tempAssistantId 
              ? { ...m, content: m.content + `\n\n❌ **세션 응답 지연 경고:** ${error.message || "서버 통신에 임시 지연이 감지되었습니다. 잠시 후 마스터 노드를 재시도합니다."}` }
              : m
          );
        } else {
          return [
