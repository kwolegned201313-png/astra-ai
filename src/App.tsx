    ...
    const loadCloudData = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          if (cloudData.imageHistory) setImageHistory(cloudData.imageHistory);
          if (cloudData.currentVideo) setCurrentVideo(cloudData.currentVideo);
          if (cloudData.generatedCode) setGeneratedCode(cloudData.generatedCode);
        }
      } catch (e) {
        ...
        <div className="flex items-center gap-2">
          {isPremium ? (
            <div 
              ...
              className="bg-amber-100/80 border border-amber-200 text-amber-800 font-bold text-xs px-3 py-1.5 rounded-2xl flex items-center gap-1 cursor-pointer hover:bg-amber-200/80 transition shadow-xs"
            >
